...
import { PamAccountPolicyRuleType, TPamAccount, useGetPamAccountPolicy } from "@app/hooks/api/pam";
...
export const ReasonGate = ({ account, children }: Props) => {
  ...
  const { data: policy, isPending: isPolicyQueryPending } = useGetPamAccountPolicy(
    account.policyId
  );
  const isPolicyLoading = Boolean(account.policyId) && isPolicyQueryPending;
  ...
  const isReasonRequired = Boolean(
    policy?.isActive && policy.rules?.[PamAccountPolicyRuleType.RequireReason]
  );
  ...
  return (
    <div className="flex h-screen w-screen flex-col items-center justify-center gap-3 bg-bunker-800">
      {isPolicyLoading ? (
        <p className="text-xs text-mineshaft-400">Loading account policy...</p>
      ) : (
        <>
          ...
          <div className="flex w-full max-w-sm flex-col gap-2">
            ...
            <div className="flex justify-center gap-2">
              <Button
                variant="outline"
                size="xs"
                isDisabled={isReasonRequired && trimmed.length === 0}
