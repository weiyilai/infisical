    ...
    <Tab title="Infisical UI">
        <Steps>
            ...
            <Step title="Configure Sync Options">
                ...
                <Note>
                    Private environment variables (`public: false`) on Travis CI are not returned by the v3 API and therefore cannot be imported. Only public environment variables will appear in Infisical after an import. If `Disable Secret Deletion` is not enabled, any private environment variables that fall within the sync's scope will be deleted.
                </Note>
